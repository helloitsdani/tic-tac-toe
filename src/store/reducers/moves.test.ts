...
describe('movesReducer', () => {
  it('returns the current state in response to an unknown action', () => {
    ...
    expect(
      movesReducer(currentState, unknownAction)
  ...
  it('responds to game reset action with the default state', () => {
    ...
    expect(
      movesReducer(currentState, gameResetAction)
    ).toEqual(defaultState)
  })

  it('updates the current state in response to a place piece action', () => {
    const placePieceAction = placePiece(
      PlayerId.PLAYER_TWO,
    ...
    const expectedState = {
      ...currentState,
      PLAYER_TWO: [
        ...currentState.PLAYER_TWO,
        BoardPosition.BOTTOM_LEFT
      ]
    }

    expect(
      movesReducer(currentState, placePieceAction)
